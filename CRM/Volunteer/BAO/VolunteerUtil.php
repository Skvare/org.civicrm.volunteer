...
class CRM_Volunteer_BAO_VolunteerUtil {
  ...
  /**
   * Assets and settings needed to boot the Backbone/Marionette volunteer app.
   *
   * @return array
   *   Keyed with "css," "templates," "prerequisite_scripts," "scripts"
   *   and "settings."
   */
  public static function loadBackbone(): array {
    $results = array(
      'css' => array(),
      'templates' => array(),
      'prerequisite_scripts' => array(),
      'scripts' => array(),
      'settings' => array(),
    );
    ...
    // Cache codes matter here: these URLs are fetched with CRM.$.getScript /
    // loadStyleFile on every dialog open, and jQuery defaults to cache:false,
    // which appends a fresh timestamp and defeats browser caching entirely. A
    // cache code gives a stable URL that still busts on a CiviCRM resource flush.
    $results['css'][] = $ccr->getUrl('org.civicrm.volunteer', 'css/volunteer_app.css', TRUE);

    // Angular Profiles previously loaded these assets as an incidental side
    // effect. CiviVolunteer's management dialogs use Backbone/Marionette
    // independently, so they must be declared and loaded here in order.
    $results['prerequisite_scripts'] = array(
      $ccr->getUrl('civicrm.packages', 'backbone/json2.js', TRUE),
      $ccr->getUrl('civicrm.packages', 'backbone/backbone.js', TRUE),
      $ccr->getUrl('civicrm.packages', 'backbone/backbone.marionette.js', TRUE),
      $ccr->getUrl('org.civicrm.volunteer', 'js/backbone/crm-bb-bootstrap.js', TRUE),
    );

    // Keep dependency order explicit. This avoids filesystem-dependent glob
    // ordering and prevents newly added development files from loading merely
    // because they happen to be under js/backbone.
    $scripts = array(
      'js/backbone/apps/volunteer_app.js',
      'js/backbone/entities/assignments.js',
      'js/backbone/entities/contacts.js',
      'js/backbone/entities/needs.js',
      'js/backbone/entities/vol_custom_field.js',
      'js/backbone/apps/_utils/renderutils.js',
      'js/backbone/apps/assign/assign_views.js',
      'js/backbone/apps/assign/assign_controller.js',
      'js/backbone/apps/define/define_views.js',
      'js/backbone/apps/define/define_controller.js',
      'js/backbone/apps/search/search_views.js',
      'js/backbone/apps/search/search_controller.js',
    );
    foreach ($scripts as $fileName) {
      $results['scripts'][] = $ccr->getUrl('org.civicrm.volunteer', $fileName, TRUE);
    }
    ...
    $roleOptions = \Civi::entity('VolunteerNeed')->getOptions('role_id', array(), TRUE, FALSE, NULL, TRUE) ?? array();
    $statusOptions = \Civi::entity('Activity')->getOptions('status_id', array(), TRUE) ?? array();

    $results['settings'] = array(
      'pseudoConstant' => array(
        'volunteer_need_visibility' => array_column($visibilityOptions, 'id', 'name'),
        'volunteer_role' => array_column($roleOptions, 'label', 'id'),
        'volunteer_status' => array_column($statusOptions, 'name', 'id'),
      ),
      // This API satisfies generic dependencies needed to build the
      // backbone-based volunteer UIs inside an Angular app. Project-specific
      // information does not belong here, so the default date is noon
      // tomorrow until a project-aware mechanism exists.
      'volunteer' => array(
